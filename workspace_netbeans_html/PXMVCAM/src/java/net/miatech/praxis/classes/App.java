...
public class App extends Application {
    ...
    public ArrayList<String>  SQP06022( String user ) throws SQLException , Exception {
        //MANT. LOG TABLE   
        CallableStatement cstmt = null;        
        ArrayList<String> resp = new ArrayList();
        String SQLCLL01 = "{CALL PRAXIS.SQP06022(?)}";
        Connection cnx = null;
        try {
            ...
            while (rs01.next()) {
                if(!"".equals(rs01.getString("USR"))){
                    resp.add(rs01.getString("USR"));
                }
                
            }
        } catch (Exception e) {
            System.out.print("SQP03268 Message: " + e.getMessage());
            throw e;
        } finally {
            if (cstmt != null) {                
                try { cstmt.close(); } 
                catch(SQLException e) { 
                    System.out.print("SQLException -> User:" + serverSession.getUserView().getUserInfo().USR + " Message: " + e.getMessage()); 
                }
            }
