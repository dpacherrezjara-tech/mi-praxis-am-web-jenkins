...
@Controller
@Scope("request")
@RequestMapping("/AplicacionVentaPre")
public class AplPaymentPreController extends BaseController {
    private AplPaymentPreLogic logic;

    @RequestMapping(value = "/search")
    public @ResponseBody
    String search(ModelMap map, HttpServletRequest request) {
        List<SQP05193Filter> listaData;
        SQP05193Filter filter;
        filter = new SQP05193Filter();
        ...
        try {
            ...
            listaData = logic.getSQP05193Filter(filter);
    ...
    String get_aplpago(ModelMap map, HttpServletRequest request) {
        List<SQP05194Filter> listaData;
        SQP05194Filter filter;
        filter = new SQP05194Filter();
//        filter.page.TOTROW = -1;
//        filter.page.START = 0;
//        filter.page.LIMIT = 0;
        try {
...
//            filter.page.PAGNUM = (start / filter.page.PAGROW) + 1;            
            logic = new AplPaymentPreLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            listaData = logic.getSQP05194Filter(filter);
    ...
    String get_aplpago_detalle(ModelMap map, HttpServletRequest request) {
        List<SQP05195Filter> listaData;
        SQP05195Filter filter;
        filter = new SQP05195Filter();
        filter.page.TOTROW = -1;
        filter.page.START = 0;
        filter.page.LIMIT = 0;
        try {                        
            ...
            logic = new AplPaymentPreLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            listaData = logic.getSQP05195Filter(filter);
            map.put("success", true);
            map.put("total", listaData.size() > 0 ? listaData.get(0).page.TOTROW : 0);            
            map.put("data", listaData);
        } catch (NumberFormatException ex) {
