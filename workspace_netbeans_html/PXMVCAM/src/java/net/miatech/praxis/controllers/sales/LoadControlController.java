...
public class LoadControlController extends BaseController {
    ...
    @RequestMapping(value = "/searchVoid")
    public @ResponseBody
    String searchVoid(ModelMap map, HttpServletRequest request) {
        List<PX019S01A1698Filter> listaData;
        filter = new PX019S01A1698Filter();
        filter.page.TOTROW = -1;
        filter.page.START = 0;
        ...
        try {
            ...
            logic = new LoadControlLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            listaData = logic.loadSQP05166(filter);
...
//                cell61.setCellValue((String)hm.get("A1698FREGI"));
    ...
    // Exportar EXCEL
    @RequestMapping(value = "/getXLSXVoid")
    public @ResponseBody
    void getXLSXVoid(HttpServletRequest request, HttpServletResponse response) {
        PX019S01A1698Filter filter = new PX019S01A1698Filter();
        try {
            Functions.msjConsola("PRAXIS", this.serverSession.getServerSession().getUserView().getUserInfo().USR, getClass().getSimpleName() + " : " + Thread.currentThread().getStackTrace()[1].getMethodName());
            filter = new Gson().fromJson(request.getParameter("beanString"), filter.getClass());
            ...
            logic = new LoadControlLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            List<PX019S01A1698Filter> listaData = logic.loadSQP05166(filter);
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
    ...
    @RequestMapping(value = "/loadErrorVoid")
    public @ResponseBody
    String loadErrorVoid(ModelMap map, HttpServletRequest request) {
        List<PX019S01A1697Filter> listaData;
        filter4 = new PX019S01A1697Filter();
        filter4.page.TOTROW = -1;
        filter4.page.START = 0;
        ...
        try {
            ...
            logic = new LoadControlLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            listaData = logic.loadSQP05167(filter4);
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
    ...
    // Exportar EXCEL err
    @RequestMapping(value = "/getXLSX_errVoid")
    public @ResponseBody
    void getXLSX_errVoid(HttpServletRequest request, HttpServletResponse response) {
        PX019S01A1697Filter filter = new PX019S01A1697Filter();
        try {
            ...
            logic = new LoadControlLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            List<PX019S01A1697Filter> listaData = logic.loadSQP05167(filter);
