...
import net.miatech.praxis.logic.salesAudit.RFNDARCReasonsFormLogic;
...
@Controller
@Scope("request")
@RequestMapping("/RFNDARCReasonsForm")
public class RFNDARCReasonsFormController extends BaseController{
    private static final Logger logError = Logger.getLogger("errorLog");
     private RFNDARCReasonsFormLogic logic;
    ...
    String SearchRFNDReasaons(ModelMap map, HttpServletRequest request) {
        ...
        try {
            logic = new RFNDARCReasonsFormLogic();
            ...
            filter.A4360FAMIL = request.getParameter("A4360FAMIL").toString().trim();
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logic = new RFNDARCReasonsFormLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A4360Filter> lst = logic.SearchRFNDReasaons(filter);

            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("Reasons ARC");
            ...
            while (iter.hasNext()) {
                ...
                CH_00.setCellValue(lst.get(vi).A4360CCUST);
                CH_01.setCellValue(lst.get(vi).A4360CODRZ);
                CH_02.setCellValue(lst.get(vi).A4360FAMIL);
                CH_03.setCellValue(lst.get(vi).A4360COMES);
                CH_04.setCellValue(lst.get(vi).A4360COMEN);
                CH_05.setCellValue(lst.get(vi).A4360COMPO);
                CH_06.setCellValue(lst.get(vi).A4360COMFR);
            ...
            String fileNameDownload = String.format("Reasons ARC - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
