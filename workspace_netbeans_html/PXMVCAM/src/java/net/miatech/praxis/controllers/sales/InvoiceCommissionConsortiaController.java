...
//        System.out.println("-------------- Invoice Commission FOB : mantenimiento-------------");
    ...
    String setData(ModelMap map, @RequestParam("excelfile") MultipartFile excelfile, HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            String keyDuplic;
            String msjDuplic = "";
            for (SQP00802Filter p : lstData) {
                keyDuplic = p.VP_A2447IATA.trim() + "|" + p.VP_A2447NFACT.trim();
                if (mapDuplic.containsKey(keyDuplic)) {
                    msjDuplic += "Line " + p.VP_A2447SEQ + ": Invoice Number '" + p.VP_A2447NFACT.trim() + "' is duplicated for IATA '" + p.VP_A2447IATA.trim() + "' (see Line " + mapDuplic.get(keyDuplic) + ")\r\n";
                } else {
                    mapDuplic.put(keyDuplic, p.VP_A2447SEQ);
                }
            }
            if (!msjDuplic.equals("")) {
                map.put("success", true);
