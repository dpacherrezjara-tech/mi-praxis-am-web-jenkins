...
//            }
    ...
    public @ResponseBody
    ResponseEntity login(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
        ...
        try {
            ...
            if (serverSession.getServerSession() == null) {
                ...
                //user.USR = request.getParameter("username"); //request.getParameter("txtAuthName");
                //user.TOKEN = request.getParameter("txtAuthPass");
                
                String username = (String) request.getParameter("username");
                
                String[] credenciales = this.getByCorreo(username);
                //List<String> groups = (Arrays.asList(request.getParameter("groups").toString().split(",")));
                
                if(credenciales.length > 0){
                    user.USR = credenciales[0];
                    user.TOKEN = credenciales[1];
                } else {
                    user.USR = "";
                    user.TOKEN = "";
                    strResponse="Mail or username is not register, please contact IT support";
                ...
                user.ID_PROFILE = groups;
                fileINF020.ID_PROFILE = user.ID_PROFILE;
                ...
                if (bValidacion) 
                {
                    ...
                    else
                    {
                        ...
                        { //Si existe conexión.
                            ...
                            PX041S01INF001Filter objUsuario = new PX041S01INF001Filter();
                            objUsuario.VP_USR = user.USR;
                            objUsuario.VP_ID_PROFILE = user.ID_PROFILE;//groups.get(0);
                            UserLogic logic = new UserLogic();
                            logic.setSession(serverSession.getServerSession());
                            List<PX041S01INF001Filter> accessUser = logic.accessUser(objUsuario);
                            request.setAttribute("accessUser", accessUser);
                            ...
                            headers.add("Location","/AEROMEXICO");
            ...
            return new ResponseEntity<>(strResponse, HttpStatus.BAD_REQUEST);
...
//        String mtypeMenu = customerInfo.MTYPE + "";
    ...
    String getAccessUser(ModelMap map, HttpServletRequest request) throws Exception {
        ...
        PX041S01INF001Filter objUsuario = new PX041S01INF001Filter();
        objUsuario.VP_USR = getCustomerInfo().USR.trim();
        objUsuario.VP_ID_PROFILE = serverSession.getServerSession().getUserView().getUserInfo().ID_PROFILE;
        UserLogic logic = new UserLogic();
        logic.setSession(serverSession.getServerSession());
        List<PX041S01INF001Filter> accessUser = logic.accessUser(objUsuario);
        request.setAttribute("accessUser", accessUser);
    ...
    /**
     * READ (GET): Busca un registro por correo (ignora mayúsculas/minúsculas).
     * Retorna la línea completa o un mensaje si no existe.
     * @param correoBuscado
     * @return [Usuario, Token]
     */
    public String[] getByCorreo(String correoBuscado) {
        try (BufferedReader reader = new BufferedReader(new FileReader("\\\\px\\PRAXISAM\\PERMITS.txt"))) {
            String line;
            UserLogic logic = new UserLogic();
            while ((line = reader.readLine()) != null) {
                String[] data = line.split(",");
                if (data.length >= 3) {
                    String correo = logic.decrypt(data[0]).trim();
                    String usuario = logic.decrypt(data[1]).trim();
                    String contrasena = logic.decrypt(data[2]).trim();
        ...
        return new String[] {}; // Retorna null si no se encuentra
    }
}
