 ...
 * @author lmendoza
 */
public class AccountingMasterControlDAO {
    ...
    public AccountingMasterControlDAO() {
    }
    ...
    public AccountingMasterControlDAO(IServerSession ss) {
        session = ss;
    }
	...
	List<A4992Filter> lstRtn = new ArrayList(0);
	A4992Filter objRtn;
	int PAGINIT = 1, totPAGS = 0, totRowsPag = filter.page.PAGROW, totRows = -1;
	...
	try {
		...
		while (rs01.next()) {
			pos++;
			objRtn = new A4992Filter();
			objRtn.RN = rs01.getLong("RN");
			objRtn.A4492ID = rs01.getString("A4492ID").trim();
			objRtn.A4492MODUL = rs01.getString("A4492MODUL").trim();
			objRtn.A4492PROG = rs01.getString("A4492PROG").trim();
			objRtn.A4492FPROC = rs01.getString("A4492FPROC").trim();
                        objRtn.A4492FCONT = rs01.getString("A4492FCONT").trim();
			objRtn.A4492DESC = rs01.getString("A4492DESC").trim();
			objRtn.A4492UREGI = rs01.getString("A4492UREGI").trim();
			objRtn.A4492FREGI = rs01.getString("A4492FREGI").trim();
			objRtn.A4492HREGI = rs01.getString("A4492HREGI").trim();
