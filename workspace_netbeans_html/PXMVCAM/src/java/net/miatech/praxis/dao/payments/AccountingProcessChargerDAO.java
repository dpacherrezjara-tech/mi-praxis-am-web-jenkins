 ...
 * @author gsanchez
 */
public class AccountingProcessChargerDAO {
    ...
    public AccountingProcessChargerDAO() {
    }
    ...
    public List<A1955Filter> search(A1955Filter filter) throws SQLException, Exception {
        ...
        strSQL = "{CALL PRAXIS.SQP00892_TMP(?,?,?,?,?,?,?,?,?)}";
        try {
    ...
    public String accountMaintance(A1955Filter filter, String strOption) throws SQLException, Exception {
        String STR_RESULT = "";
        try {    
            strSQL = "{CALL " + session.getMainLibrary() + ".SQP00902_TMP2(?,?,?,?,?,?,?,?,?)}"; 
            cnx = session.getCNXIBMDB2().getIBMDB2Connection();  
            cs = cnx.prepareCall(strSQL);
