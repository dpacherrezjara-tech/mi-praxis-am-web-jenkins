...
var LarSyrExt = function () {
    ...
    this._loadedScripts = {};
    this.loadScriptOnce = function (src) {
        if (!global._loadedScripts[src]) {
            global._loadedScripts[src] = new Promise(function (resolve, reject) {
                ...
                script.onerror = function () {
                    delete global._loadedScripts[src];
                    reject(new Error('Failed to load script: ' + src));
                };
                document.head.appendChild(script);
            });
        }
        return global._loadedScripts[src];
    };
    this.unzipFile = async function (file) {
        await global.loadScriptOnce(CONTEXTPATH + '/resources/js/JSZip/jszip.min.js');
        ...
        for (const path in zip.files) {
            ...
            entries.push({folder: folder, filename: filename, content: content});
        }
        return {zipName: file.name, entries: entries};
    };
};
