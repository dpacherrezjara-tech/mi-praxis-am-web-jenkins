Ext.define('Ext.Praxis.controller.payments.AccountingProcessCharger.DataEntryAccountingProcessChargerController',{
    extend: 'Ext.app.ViewController',
    alias: 'controller.DataEntryAccountingProcessChargerController',
...
//                Ext.getCmp(prototype.id + '-obj').focus();
    ...
    mostrarData: function(rec) {
        ...
        switch (rec.get('A1955MODUL')) {
            case 'PCHARGER': case 'PFLOWN': case 'PADJMA': case "PSETTLE": 
                Ext.getCmp(prototype.id+'-boxFecha').show();
    ...
    crud: function() {
        ...
        Ext.Ajax.request({
            ...
            beforerequest: Ext.getCmp('DataEntryAccountingProcessChargerForm').mask('Loading...'),
            success: function(response, options) {
                var res = Ext.JSON.decode(response.responseText);
                if (res.success) {
                    var msg = res.intResult;                    
                    /*var cbxModulo = mod.getValue('cbxModulo');
                    if(cbxModulo==='PCHARGER')
                    {
                    ...
                    global.Msg({
                        ...
                        fn: function() {
                            if (msg==='RECORD INSERTED') {
                                Ext.getCmp('DataEntryAccountingProcessChargerForm').close(),
                ...
                Ext.getCmp('DataEntryAccountingProcessChargerForm').unmask();
            },
    ...
    setReverse: function(objDT){
        Ext.Ajax.request({
            ...
            success: function(response, options) {
                var res = Ext.JSON.decode(response.responseText);
                if (res.success) {                    
                    Ext.create('Ext.Praxis.view.payments.AccountingProcessChargerForm.DataEntryReverse', {
                        id: 'DataEntryReverseAccountingProcessChargerForm',
                        params: {
                            rec: res.data,
                            obj: objDT.data
                        }
