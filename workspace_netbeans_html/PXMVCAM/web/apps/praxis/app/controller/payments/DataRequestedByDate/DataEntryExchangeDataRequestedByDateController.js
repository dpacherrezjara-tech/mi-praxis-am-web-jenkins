Ext.define('Ext.Praxis.controller.payments.DataRequestedByDate.DataEntryExchangeDataRequestedByDateController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.DataEntryExchangeDataRequestedByDateController',
...
////                Ext.getCmp(prototype.id + '-btn-cancel-2').show();
////                break;
            case 'V':
                this.getData();
                Ext.getCmp(prototype.id + '-btn-cancel-2').show();
...
//        console.log(beanTemp);
    ...
    getData: function() {
        ...
        Ext.Ajax.request({
            ...
            method: 'POST',
            timeout: 60000000,
            beforerequest: Ext.getCmp(prototype.id + '-dataEntryExchange').mask('Loading...'),
            params: {beanString: beanString},
            success: function(response, options) {
                Ext.getCmp(prototype.id + '-dataEntryExchange').unmask('Loading...');
                var res = Ext.JSON.decode(response.responseText);
                ...
                if(res.data.length > 0){
                    ...
                    Ext.getCmp(prototype.id + '-adgTitFechaExch').setText(titIN_DATE);
                    
                    var storeData = Ext.create('Ext.data.Store', {
                        data: res.data,
                        autoLoad: true
                    });
                    Ext.getCmp(prototype.id + '-gridExchange').bindStore(storeData);

                }else{
                    global.Msg({msg: 'Data not found.'});
                    Ext.getCmp(prototype.id + '-btn-cancel-2').fireEvent('click', {});
                }
...
// </editor-fold>
    ...
    viewTicket: function(obj, metaData, rowNum, columnNum, obj2, rowData) {
        ...
        var strTkt = rowData.data.TICKETX;
