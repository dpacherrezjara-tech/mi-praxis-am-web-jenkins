...
//            }
    ...
    btnPayment_clickHandler: function () {
        ...
        console.log(this.bean);
        this.loadTicketPayment(this.bean);
    },
...
//         controller.initial_admAccountingViewTicket(beanADMAccountig);
    ...
    imgSearchTKT_clickHandler: function (cmp, a, event) {
        var p = '';
        switch (cmp.id) {
            case prototype.id + '-imgSearchTKT1':
                p = 'TKT1';
                break;
            case prototype.id + '-imgSearchTKT2':
                p = 'TKT2';
                break;
            case prototype.id + '-imgSearchTKT3':
                p = 'TKT3';
                break;
            case prototype.id + '-imgSearchTKT4':
                p = 'TKT4';
                break;
        }
        var strTKT = '';
    ...
    imgSearch_clickHandler: function (obj, e) {
        ...
        if (win.getValue('txtFilterTicketCia').trim().length === 3 && win.getValue('txtFilterTicketFormSer').trim().length === 10) {
			...
			this.bean.SCARDN = win.getValue('lblFOP1').trim();
            console.log("tarjeta", this.bean.SCARDN);
            Ext.getCmp(prototype.id+'-btnPurge').hide();
            Ext.getCmp(prototype.id+'-vskDataPurge').hide();
        ...
        Ext.Ajax.request({
            ...
            success: function (response, options) {
                ...
                //            with(image){x=0; y=0}
                        ...
                        if (me01.filterTKT.lstResultSet01.length > 1)
                            Ext.getCmp(prototype.id + '-boxCoupons').setHeight(Ext.getCmp(prototype.id + '-boxCoupons').getHeight() + 81);
                        var strTKTIND = '';
                        ...
                        for (var i = 0; i < me01.filterTKT.lstResultSet01.length; i++) {
                            ...
                            if (i > 0)
                                win.setValue('lblTicketNumberConjuntion', win.getValue('lblTicketNumberConjuntion') + ((i > 1) ? '-' : '') + strTKTIND); //Exist Conjuntion TKT.
                            ...
                            for (var i16 = 0; i16 < me01.filterTKT.lstResultSet15.length; i16++) {
                                ...
                                if (paramsResultSet15.fileA1747.SEQ.trim() !== '' && paramsResultSet15.fileA1747.SEQ.trim() !== 'XX')
                                {
                                    ...
                                    me01.gridDataTktRealUsesAC.push({
                                            ...
                                            REF : paramsResultSet15.fileA1747.CUPON === '1' ? (paramsResultSet15.fileA1747.A4373CUPN1.trim() === '' ? '':'RFTX') :
                                              paramsResultSet15.fileA1747.CUPON === '2' ? (paramsResultSet15.fileA1747.A4373CUPN2.trim() === '' ? '':'RFTX') :
                                              paramsResultSet15.fileA1747.CUPON === '3' ? (paramsResultSet15.fileA1747.A4373CUPN3.trim() === '' ? '':'RFTX') :
                                              (paramsResultSet15.fileA1747.A4373CUPN4.trim() === '' ? '':'RFTX'),
                                            STAT : 'DISC',
...
//                                        FARE : paramsResultSet23.fileA1692.FBASE
                        ...
                        if (me01.filterTKT.lstResultSet07.length > 0) {
                            for (var i7 = 0; i7 < me01.filterTKT.lstResultSet07.length; i7++) {
                                ...
                                me01.bean.SCARDN = paramsResultSet07.fileA1531.A1531NREF.substr(0, 16);
                                
                                switch (i7) {
                                    case 0:
                                        me01.bean.SCARDNpos = 1;
    ...
    loadTicketPayment: function (bean) {
        var me01 = this;
        prototype.url = URL_VIEWTICKET;
        Ext.Ajax.request({
            url: prototype.url + '/loadPayment',
            method: 'POST',
            timeout: 60000000,
            params: {beanString: JSON.stringify(bean)},
            beforerequest: Ext.getBody().mask('Loading...'),
            success: function (response, options) {
                ...
                if (res.success) {
                    //win.setValue('txtFilterTicketSeq', '');
                    me01.filterPayment = res.filterPayment;
                    var dataEntryADM = Ext.create('Ext.Praxis.view.program.ProMasterTicketForm.DataEntryPayment', {id: 'DataEntryPaymentDevForm'});
                    var controller = dataEntryADM.getController();
                    controller.gridDataMemoAC = res.filterPayment;
                    dataEntryADM.show();
