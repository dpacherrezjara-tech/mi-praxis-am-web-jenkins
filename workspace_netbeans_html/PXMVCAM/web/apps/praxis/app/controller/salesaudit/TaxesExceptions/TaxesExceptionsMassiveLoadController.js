Ext.define('Ext.Praxis.controller.salesaudit.TaxesExceptions.TaxesExceptionsMassiveLoadController', {
    ...
    readExcel: async function (file) {
        ...
        await global.readExcelFile(file, async function (data) {
            let errors = 0;
            const fileLayout = ['TICKET', 'SALEDATE', 'TAXCODE', 'COMMENT'];
            data.forEach(x => {
                const claves = Object.keys(x);
                const isValid = claves.length === fileLayout.length &&
                        fileLayout.every(clave => claves.includes(clave));
                if (!isValid) {
                    errors++;
                }
            });
            ...
            if (errors > 0) {
                notifier.alert('Invalid Layout, total Errors: ' + errors,{durations: {alert: 9000}});
                return;
            }
