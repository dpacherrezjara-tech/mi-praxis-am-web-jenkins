Ext.define('Ext.Praxis.controller.salesaudit.BPOControlAnalytics.GraphicsRankingController', {
    ...
    loadDataFromGrid: function () {
        ...
        let totalSol = chartData.reduce(function (sum, r) {
            return sum + r.SOL;
        }, 0);
        let avgSol = (totalSol / chartData.length).toFixed(1);
        let totalPromMin = chartData.reduce(function (sum, r) {
            return sum + r.PROM_MIN;
        }, 0);
        let avgPromMin = (totalPromMin / chartData.length).toFixed(2);
        let avgEficiencia = (chartData.reduce(function (sum, r) {
            return sum + r.EFICIENCIA_PCT;
        }, 0) / chartData.length).toFixed(1);

        // Encontrar el mejor y peor
        let maxSol = Math.max.apply(null, chartData.map(function (r) {
            return r.SOL;
        }));
        let minPromMin = Math.min.apply(null, chartData.map(function (r) {
            return r.PROM_MIN;
        }));
        let maxEficiencia = Math.max.apply(null, chartData.map(function (r) {
            return r.EFICIENCIA_PCT;
        }));

        let bestAgent = chartData.find(function (r) {
            return r.SOL === maxSol;
        });
        let fastestAgent = chartData.find(function (r) {
            return r.PROM_MIN === minPromMin;
        });
        let mostEfficient = chartData.find(function (r) {
            return r.EFICIENCIA_PCT === maxEficiencia;
        });
    ...
    /**
     * Descargar gráfico dual
     */
    onDownloadDualChart: function () {
        this.downloadChart(
                'dualChart',
                'Comparison and Dual Analysis'
                );
    },

    downloadChart: function (chartRef, fileName) {

        let me = this;
        let chart = me.lookup(chartRef);

        if (!chart) {
            Ext.Msg.alert('Error', 'Chart not found');
            return;
        }

        Ext.Msg.wait('Generating image...', 'Please wait');
