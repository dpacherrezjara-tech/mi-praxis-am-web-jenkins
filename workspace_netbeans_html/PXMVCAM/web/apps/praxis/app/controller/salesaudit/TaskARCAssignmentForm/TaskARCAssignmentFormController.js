...
Ext.define('Ext.Praxis.controller.salesaudit.TaskARCAssignmentForm.TaskARCAssignmentFormController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.TaskARCAssignmentFormController',
...
//        Ext.getCmp(prototype.id + '-pagginator-01').setStore(store00);
    ...
    onSearchClick: function (btn) {
        ...
        me.beanTMP.IN_FOLIO = Ext.String.trim(Ext.getCmp(prototype.id + '-txtNumber').getValue());
...
//        }
    ...
    OnDetail01: function (rowIndex) {
        ...
        if (rec.get('A4361REGAS')) {
            if (rec.get('A4361CANTPEDI') > 0) {
                Ext.getCmp(prototype.id + '-txtUser').setValue(rec.get('A4361REGAS'));
                ...
                Ext.getCmp(prototype.id + '-btn-excel').hide();
                ...
                Ext.getCmp(prototype.id + '-txtIATA').disable();
                Ext.getCmp(prototype.id + '-txtUser').disable();

                vl_fechaini = rec.get('A4361FREGI').substring(0, 4) + '' + this.getDataMes(rec.get('A4361FREGI').substring(7, 4)) + '01';
                vl_fechafin = rec.get('A4361FREGI').substring(0, 4) + '' + this.getDataMes(rec.get('A4361FREGI').substring(7, 4)) + '31';
                ...
                Ext.Msg.alert('.: PRAXIS :.', 'It must have pending records in order to proceed with the reassignment');
                return;
            }
    ...
    onBackClick: function (obj) {
        ...
        if (grid02.isVisible()) {
            ...
            Ext.getCmp(prototype.id + '-btn-excel').show();
            Ext.getCmp(prototype.id + '-txtIATA').enable();
            Ext.getCmp(prototype.id + '-txtUser').enable();
    ...
    onChangeAuditorClick: function (obj) {
        ...
        if (newAuditor === 'ALL' || newAuditor === 'AUTOAM' || newAuditor === 'AUTOPR' || newAuditor === null) {
            newAuditor = '';
        }
        ;
        if (newAuditor === '') {
            Ext.Msg.alert('.: PRAXIS :.', 'You must enter the Auditor to be reassigned');
            return;
        }
        if (grid.getSelectionModel().hasSelection()) {
            ...
            for (var i = 0; i < selection.length; i++) {
                ...
                if (Ext.String.trim(row.get('A4361FLAG')) === 'R' || Ext.String.trim(row.get('A4361FLAG')) === 'F') {
                    Ext.Msg.alert('.: PRAXIS :.', 'The records cannot be reassigned because they have already been processed');
                    return;
                } else {
                    if (newAuditor === Ext.String.trim(row.get('A4361REGAS'))) {
                        Ext.Msg.alert('.: PRAXIS :.', 'You cannot reassign to the same Auditor');
                        return;
                    } else {
            ...
            if (lstNewList.length > 0) {
                global.Msg({
                    ...
                    fn: function (btn) {
                        if (btn === 'yes') {
                            me.beanGuardar.IN_USER = newAuditor;
                ...
                Ext.Msg.alert('.: PRAXIS :.', 'Debes selecionar una Solicitud');
                return;
            }

        } else {
            Ext.Msg.alert('.: PRAXIS :.', 'You must select a request aaa');
            return;
        }
