Ext.define('Ext.Praxis.controller.flown.FlightManifest.FlightManifestController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.FlightManifestController',
    childs: '',
    stack: [],
    bean: {},
    bean2149: {},
...
//        }
    ...
    winDataEntry: function (action, rec) {
        ...
        Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntryA3729', {
...
//        var chkManifest = this.getValue("chkManifest");
        ...
        if (this.getValue("txtTKT") !== '' || this.getValue("txtCDEPART") !== '' || this.getValue("txtLNAME") !== '' || this.getValue("txtCHAIR") !== '' || this.getValue("txtFlight") !== '' ) {
            ...
            this.objFLIGHTDETAIL.monthTo = this.getValue("cmbDateToMonth");
            this.objFLIGHTDETAIL.dayFrom = this.getValue("cmbDateFromDay");
...
//                });
    ...
    btnAdd_click: function () {
        if (this.peek() === prototype.id + '-boxDetTicket') {
            Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntryTicket', {
                id: 'DataEntryTicketFlightManifestForm',
                params: {
                    actionCode: 'I'
                }
            }).show();
        } else if (this.peek() === prototype.id + '-boxDetailData') {
            Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntry', {
                id: 'DataEntryFlightManifestForm',
                params: {
                    actionCode: 'I'
                }
            }).show();
        ...
        if (Ext.getCmp(prototype.id + '-boxPrincipal').isVisible()) {
            if (this.peek() === prototype.id + '-boxMainData') {
                ...
                if (data.length !== 0) {
                    ...
                    Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntryQueryTicket', {
                        id: 'DataEntryQueryTicketFlightManifestForm',
                        params: {
                            bean: beanQuery92
                        }
                    }).show();
...
//                    Ext.getCmp(prototype.id + '-boxDetailData').mask('Loading...');
                ...
                load: function (obj, obj2, success, response, obj5) {
                    ...
                    if (!me.peek().includes('boxDetailData'))
                        ...
                        me.selectedChild('boxDetailData', '', false);
...
//                        me.cargarTicket();
    ...
    onChangeChkObs: function () {
        if (me.stack[me.stack.length - 1 ] === 'FlightManifestForm-boxDetailData') {
            if (this.getValue('chkObs')) {
                this.bean.IN_OBS = 'Y'
            } else {
                this.bean.IN_OBS = ''
...
//        Ext.getCmp(prototype.id + '-btnScanTicket').setVisible(true);
        console.log('entra al searchDetail')
        var storeGridDatas = Ext.create('Ext.Praxis.store.flown.GridData', {
            proxy: {
            ...
            listeners: {
                ...
                load: function (obj, obj2, success, obj4, obj5) {
                    console.log('load search detail')
                    Ext.getCmp(prototype.id + '-boxDetailData').unmask();
...
//                      global.Msg({msg: 'This File has not been created.'});
    ...
    openExport: function (grid, rowIndex, colIndex, a, b, c) {
        ...
        Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntryExport', {
            id: 'DataEntryExportFlightManifestForm',
            params: {
                strRuta: RutaF,
                fecha: fecha,
                nFlight: nFlight
    ...
    exportFile1: function (ruta, fecha, nFlight) {
        Ext.Ajax.request({
            ...
            beforerequest: Ext.getCmp('DataEntryExportFlightManifestForm').mask('Loading...'),
            success: function (response, opts) {
                Ext.getCmp('DataEntryExportFlightManifestForm').unmask();
                var res = Ext.JSON.decode(response.responseText);
                if (res.success) {
    ...
    buscarDatosVenta: function (beanOption) {
        Ext.Ajax.request({url: prototype.url + '/buscarDatosVenta',
            ...
            beforerequest: Ext.getCmp('DataEntryTicketFlightManifestForm').mask('Loading...'),
            success: function (response, opts) {
                Ext.getCmp('DataEntryTicketFlightManifestForm').unmask();
                win.lblUser_toolTip("Estructura: A1692");
                var res = Ext.JSON.decode(response.responseText);
                if (res.success) {
                    var msj = res.msjOption;
...
//            Ext.getCmp(prototype.id + '-pie').setWidth(width);
    ...
    viewTicket: function (obj, metaData, rowNum, columnNum, obj2, rowData) {
        ...
        win.displayProMasterTicket(this, 'ViewFlightManifest', beanProMasterTicket);
    },
...
//	}
        ...
        win.displayProFacsimilSearch(me, bean104, 'FlightManifest');
    },
    ...
    btnDuplicate_click: function () {
        ...
        Ext.create('Ext.Praxis.view.flown.FlightManifestForm.DataEntryDelete', {
//            id: 'DataEntryDeleteFlightManifestForm',
            id: prototype.id + '-DataEntryDeleteFlightManifestForm',
            params: {
                strFecha: '',
                strFuente: '',
                strccust: ''
