...
Ext.define('Ext.Praxis.controller.sales.InvoiceCommissionConsortiaContract.DataEntryInvoiceCommissionConsortiaContractController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.' + prototype.id + '-dataEntryController',
    url: CONTEXTPATH + '/InvoiceCommissionConsortiaContract',
    lblPreffixOld: '',
    /**
     * Constructor
     */
...
//            Ext.getCmp(prototype.id + '-txtA1757IVACB_P').setValue(Ext.util.Format.number(data.A2444IVA16, '0,000.00'));    //COLHDG('IVA COMM BANCARIA')
    ...
    crud: function () {
        ...
        Ext.Ajax.request({
            ...
            beforerequest: Ext.getCmp(prototype.id + '-DataEntryInvoiceCommissionConsortiaContractForm').mask('Loading...', ''),
            success: function (response, options) {
                var res = Ext.JSON.decode(response.responseText);
                var objRtn = res.objRtn;
                console.log(objRtn);
                Ext.getCmp(prototype.id + '-DataEntryInvoiceCommissionConsortiaContractForm').unmask('Loading...', '');
                global.Msg({
                    msg: objRtn.dbException.MESSAGE,
                    icon: 1,
                    fn: function () {
                        //exito
                        Ext.getCmp(prototype.id + '-DataEntryInvoiceCommissionConsortiaContractForm').close();
    ...
    getTotalPref: function () {
        //DVT valida si codigo no existe
        if (Ext.getCmp(prototype.id + '-txtA1757COD').getValue() !== '') {
            Ext.getCmp(prototype.id + '-txtA1757CAMCO_P').setValue('0.00'); //COLHDG('MONTO CASH - COM')                         
...
//              console.log(result.objRtn);       
                ...
                if (result.length > 0) {
                    ...
                    if (p.action == 'I') {
                        ...
                        if (Ext.getCmp(prototype.id + '-txtA1757COD').getValue() !== '') {
                            console.log("Se ejecuta STORED de COMBA");
                            ...
                            console.log(Ext.getCmp(prototype.id + '-txtA1757COMBA_P').getValue());
...
//        console.log(preInv_Charge);
//        console.log(preInv_IvaComBa);
    ...
    onBlurEmdVal: function (obj) {
        ...
        switch (tf) {
            ...
            case 'txtA1757COMBA':
                ...
                let comba_p = Ext.Number.parseFloat(Ext.getCmp(prototype.id + `-${tf}_P`).getValue().replace(",", "").replace(",", ""));
