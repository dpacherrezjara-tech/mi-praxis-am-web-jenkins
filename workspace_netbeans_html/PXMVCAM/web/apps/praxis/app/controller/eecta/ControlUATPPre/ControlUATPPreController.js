Ext.define('Ext.Praxis.controller.eecta.ControlUATPPre.ControlUATPPreController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.ControlUATPPreController',
    beanXLS: {},       
    me: '',
    setContext: function () {
        me = this;
...
//        });
    ...
    cmbfiltroSTS_clickHandler:function(){        
        this.onDetalleUATPPreFilter(null, 0, 0);
//        var rowIndex = Ext.getCmp(prototype.id + '-rowIndex').getValue();  
//        console.log('rowIndex:' + rowIndex);
//        if (rowIndex > 0) 
//        this.onDetalleUATPPreFilter(null, rowIndex, 0);
//        else console.log('cmbfiltroSTS_clickHandler');
    },
    onDetalleUATPPre: function (grid, rowIndex, colIndex) {
...
//    -- buscqeud a detalle x ticket y estado
    onDetalleUATPPreFilter: function (grid, rowIndex, colIndex) {
//        if (Ext.getCmp(prototype.id + '-gridData')) {
//            var grid = Ext.getCmp(prototype.id + '-gridData');
//            var store = grid.getStore();
//            var rec = store.getAt(rowIndex);
...
//        }
        var bean = {};        
        bean.VP_OPCION = '';
        bean.VP_FDATE1 = Ext.getCmp(prototype.id + '-FCONT').getValue();
        bean.VP_TICKET = Ext.getCmp(prototype.id + '-TKT-NUMBER').getValue();
        var SEQ = Ext.getCmp(prototype.id + '-TKT-NUMBER-SEQ').getValue();
        if(bean.VP_TICKET !== '') 
        bean.VP_TICKET = bean.VP_TICKET + SEQ;
...
//        Ext.getCmp(prototype.id + '-rowIndex').setValue( rowIndex ); //usado para buscar x estados y ticket
    ...
    winDataEntry: function (action, rec) {
        ...
        Ext.create('Ext.Praxis.view.eecta.ControlUATPPreForm.ControlUATPPreProcesarForm', {
            id: prototype.id02 + '-ControlUATPPreProcesarForm',
            params: {
                action: action,
                rec: rec
            }
    ...
    winDataEntry01: function (action, rec) {
        action = action === null || action === undefined ? 'U' : action;
        rec = rec === null || rec === undefined ? {} : rec;        
        Ext.create('Ext.Praxis.view.eecta.ControlUATPPreForm.ControlUATPPreUUIDForm', {
            id: prototype.id03 + '-ControlUATPPreUUIDForm',
            params: {
                action: action,
                rec: rec
            }
    ...
    winDataEntry02: function (action, rec) {
        action = action === null || action === undefined ? 'U' : action;
        rec = rec === null || rec === undefined ? {} : rec;        
        Ext.create('Ext.Praxis.view.eecta.ControlUATPPreForm.ControlUATPPreErroresForm', {
            id: prototype.id04 + '-ControlUATPPreErroresForm',
            params: {
                action: action,
                rec: rec
            }
