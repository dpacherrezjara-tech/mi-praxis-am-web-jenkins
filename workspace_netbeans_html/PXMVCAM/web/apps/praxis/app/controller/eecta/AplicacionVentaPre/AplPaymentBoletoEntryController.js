...
Ext.define('Ext.Praxis.controller.eecta.AplicacionVentaPre.AplPaymentBoletoEntryController', {
    ...
    afterRender: function () {
        ...
        this.getDataInputs();        
        Ext.getCmp(prototype.id + '-A4252TOTPG').focus();        
    },
    ...
    getDataInputs: function () {
        ...
        Ext.getCmp(prototype.id + '-A4246NRRPT').setValue(data.A4245NRRPT);
        Ext.getCmp(prototype.id + '-A4246CDCLI').setValue(data.A4245CDCLI);        
        Ext.getCmp(prototype.id + '-A3953RSOCI').setValue(data.A3953RSOCI.trim());
        Ext.getCmp(prototype.id + '-A4252BANCO').setValue(data.A3953BANCO.trim());
        Ext.getCmp(prototype.id + '-A4252CTABC').setValue(data.A3953CTABC.trim());
        Ext.getCmp(prototype.id + '-A4252REFPG').setValue(data.A4245REFBC.trim());  
        Ext.getCmp(prototype.id + '-A4252MDAPG').setValue(data.A4245MDLOC); 
    ...
    getDataEntryValues: function (strOption) {
        ...
        var VL_A4252REFPG = Ext.getCmp(prototype.id + '-A4252REFPG').getValue();
        var VL_A4252FECPG = Ext.util.Format.date(Ext.getCmp(prototype.id + '-A4252FECPG').getValue(), 'Ymd');
        var VL_A4252TOTPG = Ext.Number.parseFloat(Ext.getCmp(prototype.id + '-A4252TOTPG').getValue().replace(",", "").replace(",", ""));        
        var VL_A4252MDAPG = Ext.getCmp(prototype.id + '-A4252MDAPG').getValue();
        var VL_A4252TIPPG = "M";
        var VL_A4252NRRPT = Ext.getCmp(prototype.id + '-A4246NRRPT').getValue();
        var VL_A4252CDCLI = Ext.getCmp(prototype.id + '-A4246CDCLI').getValue();
        var VL_A4252BANCO = Ext.getCmp(prototype.id + '-A4252BANCO').getValue();
        var VL_A4252CTABC = Ext.getCmp(prototype.id + '-A4252CTABC').getValue(); 
        var VL_TICKET_NC = Ext.getCmp(prototype.id + '-TICKET_NC').getValue(); 
        var VL_NUM_RECIBO = Ext.getCmp(prototype.id + '-NUM-RECIBO').getValue();
                
        return {
            VP_ACTION:VP_ACTION,
            A4252REFPG:VL_A4252REFPG,
            A4252FECPG:VL_A4252FECPG,
            A4252TOTPG:VL_A4252TOTPG,
            A4252MDAPG:VL_A4252MDAPG,
            A4252TIPPG:VL_A4252TIPPG,
            A4252NRRPT:VL_A4252NRRPT,
            A4252CDCLI:VL_A4252CDCLI,
            A4252BANCO:VL_A4252BANCO,
            A4252CTABC:VL_A4252CTABC,
    ...
    get_SelectedRecords:function(){
        ...
        if (grid.getSelectionModel().hasSelection()) {
            ...
            for (var i = 0; i < selection.length; i++) {
                ...
                vl_total_sel = vl_total_sel + Ext.Number.parseFloat(row.get('A4246SALDP'));
//                if ( Ext.Number.parseFloat(row.get('A4246SALDP')) > 0 ) {
//                    vl_total_sel = vl_total_sel + Ext.Number.parseFloat(row.get('A4246SALDP'));                    
//                }
                var rec_obj = {
                    A4246CCUST:row.get('A4246CCUST'),
                    A4246CIA:row.get('A4246CIA'),
                    A4246FORMA:row.get('A4246FORMA'), 
                    A4246SERIE:row.get('A4246SERIE'), 
                    A4246SEQ:row.get('A4246SEQ'),
                    A4246TRNCU:row.get('A4246TRNCU'), 
                    A4246GRUPO:row.get('A4246GRUPO'), 
                    A4246NRRPT:row.get('A4246NRRPT'), 
                    A4246SQRPT:row.get('A4246SQRPT'), 
                    A4246CDCLI:row.get('A4246CDCLI'), 
                    A4246TOT:row.get('A4246TOT'),
                    A4246SALDP:row.get('A4246SALDP')                    
                };
...
//                console.log(objRtn);
    ...
    validateForm: function (params) {
        ...
        if (params.A4252TOTPG === '' || params.A4252TOTPG === 0 ) {
        ...
        if (params.VP_TICKET_NC === ''){
            if (params.A4252FECPG === '') {
                mensaje = 'Ingrese la fecha de pago';
                Ext.getCmp(prototype.id + '-A4252FECPG').focus();
                return mensaje;
            }                               
        }
        var arrayRec = this.get_SelectedRecords();        
        var vl_total_sel = arrayRec[1];        
        if( params.A4252TOTPG > vl_total_sel ){
    ...
    get_detalle_boleto: function () {
        ...
        var p = this.view.params;                
        var bean = {};        
        bean.VP_A4246NRRPT = p.rec.data.A4245NRRPT;
        bean.VP_A4246CDCLI = p.rec.data.A4245CDCLI;        
        bean.VP_TFILTTRO = '';
