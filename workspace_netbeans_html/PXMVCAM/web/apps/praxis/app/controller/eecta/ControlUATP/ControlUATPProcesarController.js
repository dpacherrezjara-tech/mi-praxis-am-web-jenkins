...
Ext.define('Ext.Praxis.controller.eecta.ControlUATP.ControlUATPProcesarController', {
    ...
    getDataEntryValues: function (strOption) {
        ...
        var VP_CDCLI  = '';
        if (VL_PROCESO==='UATP'){
            VL_FECHA1 = Ext.util.Format.date(Ext.getCmp(prototype.id + '-FECHA1').getValue(), 'Ymd');
            VL_FECHA2 = Ext.util.Format.date(Ext.getCmp(prototype.id + '-FECHA2').getValue(), 'Ymd');
        }   
        if (VL_PROCESO==='REPT')
        VP_FEJEC = Ext.util.Format.date(Ext.getCmp(prototype.id + '-FECHEJE01').getValue(), 'Ymd');            
        
        if (VL_PROCESO==='EECC'){
            VP_FEJEC = Ext.util.Format.date(Ext.getCmp(prototype.id + '-FECHEJE02').getValue(), 'Ymd');            
            VP_CDCLI = '';            
        }                       
        return {
            VP_ACTION:VL_ACTION,
...
//        return;
    ...
    validateForm: function (params) {
        ...
        if (params.VP_PROCESO === 'REPT') {
            if(params.VP_FEJEC === '' )
            mensaje = 'INGRESE FECHA DE EMISION ';
            Ext.getCmp(prototype.id + '-FECHEJE01').focus();
            return mensaje;
        }
         if (params.VP_PROCESO === 'EECC') {
            if(params.VP_FEJEC === '' )
            mensaje = 'INGRESE FECHA DE EMISION ';
            Ext.getCmp(prototype.id + '-FECHEJE02').focus();
            return mensaje;
        }
        return mensaje;
    },
