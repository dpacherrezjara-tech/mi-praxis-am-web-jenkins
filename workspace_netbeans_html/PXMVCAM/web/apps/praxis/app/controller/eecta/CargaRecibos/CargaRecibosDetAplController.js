...
Ext.define('Ext.Praxis.controller.eecta.CargaRecibos.CargaRecibosDetAplController', {
    ...
    getDataInputs: function () {
        ...
        /*
        Ext.getCmp(prototype.id05 + '-A4103IDRCB').setValue(data.A4103IDRCB);
        Ext.getCmp(prototype.id05 + '-A4103CDCLI').setValue(data.A4103CDCLI);        
        Ext.getCmp(prototype.id05 + '-A3953RSOCI').setValue(data.A3953RSOCI.trim()); 
        Ext.getCmp(prototype.id05 + '-A3957INIPR').setValue(data.A3957INIPR); //Ext.util.Format.date(data.A3957INIPR, 'Y/m/d')
        Ext.getCmp(prototype.id05 + '-A3957FINPR').setValue(data.A3957FINPR);        
        Ext.getCmp(prototype.id05 + '-A3957TOT').setValue(Ext.util.Format.number( data.A3957TOT , '0,000.00'));
        Ext.getCmp(prototype.id05 + '-A3957MDLOC').setValue(data.A3957MDLOC);
        Ext.getCmp(prototype.id05 + '-A3957TOTAP').setValue(Ext.util.Format.number( data.A3957TOTAP , '0,000.00'));
        Ext.getCmp(prototype.id05 + '-A3957SALDP').setValue(Ext.util.Format.number( data.A3957SALDP , '0,000.00'));       
        */
       // this.get_aplpago();        
    },
...
//                    Ext.getCmp(prototype.id05 + '-lbl-total').setText(total);
        ...
        Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentCab').setStore(storeGridDatas);
        Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentCab').getStore().reload();
    },
    get_aplpago_detalle: function ( grid, rowIndex, colIndex ) { 
        var rec = [];
        Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentDet').setTitle( '' );
        var VL_IDPG = '';
        console.log('rowIndex: '  + rowIndex);
        if(rowIndex>0){
            rec = grid.getStore().getAt(rowIndex); 
            VL_IDPG = rec.data.A3959IDPG;              
            Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentDet').setTitle( 'Detalle Id pago nº: ' + rec.data.A3959IDPG ); 
            Ext.getCmp(prototype.id05 + '-TICKET-NUMB').setValue("");
        }
        var bean = {};        
        bean.VP_IDPG = VL_IDPG;        
        var vl_ticket_cia = Ext.getCmp(prototype.id05 + '-TICKET-CIA').getValue();
        var vl_ticket_num = Ext.getCmp(prototype.id05 + '-TICKET-NUMB').getValue();
        var vl_ticket_seq = Ext.getCmp(prototype.id05 + '-TICKET-SEQ').getValue();
        bean.VP_TICKET =  "";
        if (vl_ticket_num !== ""){
...
//                    Ext.getCmp(prototype.id05 + '-lbl-total').setText(total);
        ...
        Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentDet').setStore(storeGridDatas);
        Ext.getCmp(prototype.id05 + '-infoGridAppliedPaymentDet').getStore().reload();
    },
