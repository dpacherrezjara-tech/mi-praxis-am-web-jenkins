...
Ext.define('Ext.Praxis.controller.travelbank.DeliveryFiles.DeliveryFilesController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.DeliveryFilesController',
    searchParams: {},
    init: function (view) {
        prototype.id = 'DeliveryFilesForm';
        prototype.url = CONTEXTPATH + '/DeliveryFiles';
        prototype.widthContenedor = 1500;
        prototype.widthGrid = 1459;

        var me = this;
        this.control({
            '#DeliveryFilesForm-btn-pag-first': {
                click: this.pagFirst
            },
            '#DeliveryFilesForm-btn-pag-previous': {
                click: this.pagPrevious
            },
            '#DeliveryFilesForm-btn-pag-next': {
                click: this.pagNext
            },
            '#DeliveryFilesForm-btn-pag-last': {
                click: this.pagLast
            },
            '#DeliveryFilesForm-btnSearch': {
                click: this.btnSearch_click
            },
            '#DeliveryFilesForm-btnFilter': {
                click: this.btnFilter_click
            },
////            '#AccountMasterTravelBankForm-btnDisplay': {
////                click: this.btnDisplay_click
////            },
            '#DeliveryFilesForm-btnExcel': {
                click: this.btnExcel_click
            },
            '#DeliveryFilesForm-btnClear': {
                click: this.btnClear_click
            },
            '#DeliveryFilesForm-btnAdd': {
                click: this.btnAdd_click
            },
            '#DeliveryFilesForm-btnBack': {
    ...
    setFormatParameter: function () {
        let IN_CCUST = '139',
                IN_TIPO = Ext.getCmp(prototype.id + '-cboFileType').getValue(),
                IN_IDFIL = Ext.getCmp(prototype.id + '-txtIdfile').getValue(),
                IN_FECHAI = Ext.util.Format.date(Ext.getCmp(prototype.id + '-txtDateFrom').getValue(), 'Ymd'),
                IN_FECHAF = Ext.util.Format.date(Ext.getCmp(prototype.id + '-txtDateTo').getValue(), 'Ymd');
        searchParams = {
            CCUST: IN_CCUST,
            FECHAI: IN_FECHAI,
            FECHAF: IN_FECHAF,
...
//    },
    ...
    btnClear_click: function (obj, e) {
        ...
        Ext.getCmp(prototype.id + '-txtIdfile').setValue("");
