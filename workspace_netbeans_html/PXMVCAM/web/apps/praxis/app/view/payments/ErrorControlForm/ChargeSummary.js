...
Ext.define('Ext.Praxis.view.payments.ErrorControlForm.ChargeSummary', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.' + prototype.id + '-chargeSummary',
    requires: [
        'Ext.Praxis.view.payments.ErrorControlForm.ChargeGrid'
    ],
    height: 625,
    width: 1400,
    layout: 'center',
    ...
    items: [
        {
            ...
            titleAlign: 'center',
            id: prototype.id + '-gridChargeSummary',
            height: 625,
            width: 605,
            viewConfig: {
                stripeRows: true,
    ...
    getData: async function () {
        ...
        if (me.searchParams && me.searchUrl) {
            const grid = me.down();
            grid.mask('Loading...');
            const res = await fetch(`${me.searchUrl}/loadErrorArchSummary?${new URLSearchParams(me.searchParams)}`);
            if (res.ok) {
    ...
    onShowDetails: function (obj) {
        ...
        let params = {
            DATE_FROM: rec.a4297FPRDA,
            ARCHIVO: rec.a4297SOURC.trim(),
            CERROR: Ext.getCmp(prototype.id + '-cmbError').getValue()
        };
        const panelActual = me.items.items.at(-1);
        panelActual.hide();
        const detPanel = Ext.create('Ext.Praxis.view.payments.ErrorControlForm.ChargeGrid', {
            id: prototype.id + '-panelChargeGrid',
            searchParams: params,
            searchUrl: CONTEXTPATH + '/ErrorControl/loadErrorArchDetail'
        });
