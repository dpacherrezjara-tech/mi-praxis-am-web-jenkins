
prototype.idRFNDAssociatedARCR = 'RFNDAssociatedARCRFNDForm';
prototype.url = CONTEXTPATH + '/RFNDAssociatedARCRFNDForm';
prototype.widthWindow = 1366;
prototype.heightWindow = 768;

Ext.define('Ext.Praxis.view.salesaudit.RFNDAssociatedARCRFNDForm.RFNDAssociatedARCRFNDForm', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.RFNDAssociatedARCRFNDForm',

    requires: [
        'Ext.Praxis.controller.salesaudit.RFNDAssociatedARCRFNDForm.RFNDAssociatedARCRFNDFormController',
    ],

    controller: 'RFNDAssociatedARCRFNDFormController',

    id: prototype.idRFNDAssociatedARCR + '-Contenedor',

    layout: {
        type: 'vbox',
        align: 'center'
    },
    bodyStyle: 'background-color: white;',
    border: false,
    scrollable: true,
    defaults: {
        border: false
    },
    listeners: {
        beforeShow: 'OnBeforeShow'
    },
    items: [
        {
            xtype: 'panel',
            id: prototype.idRFNDAssociatedARCR + '-contenedor-form',
            width: 1366,
            items: [
                {
                    xtype: 'panel',
                    id: prototype.idRFNDAssociatedARCR + '-contenedor-options',
                    border: false,
                    bodyStyle: 'background: transparent',
                    padding: '10px 5px 0px 5px',
                    layout: {
                        type: 'hbox',
                        pack: 'end'
                    },
                    items: [
                        {
                            xtype: 'panel',
                            layout: 'hbox',
                            border: false,
                            defaults: {
                                style: 'padding: 4px; margin: 1px;'
                            },
                            items: [
                                {
                                    xtype: 'checkbox',
                                    id: prototype.idRFNDAssociatedARCR + '-pagination',
                                    boxLabel: 'Pagination?',
                                    checked: true,
                                    disabled: true,
                                    listeners: {
                                        change: 'onPaginationChkChange'
                                    }
                                },
                                {
                                    xtype: 'Paginator',
                                    id: prototype.idRFNDAssociatedARCR + '-pagginator-01',
                                    pagInfo: [
                                        prototype.idRFNDAssociatedARCR + '-lbl-currentPage',
                                        prototype.idRFNDAssociatedARCR + '-lbl-pageCount',
                                        prototype.idRFNDAssociatedARCR + '-lbl-total'
                                    ]
                                }
                            ]
                        },
                        {xtype: 'tbspacer', width: 50},
                        {
                            xtype: 'toolbar',
                            items: [
                                {
                                    xtype: 'button',
                                    id: prototype.idRFNDAssociatedARCR + '-btn-search',
                                    iconCls: 'prx-icon-search',
                                    tooltip: 'Search',
                                    listeners: {
                                        click: 'onSearchClick'
                                    }
                                },
                                {
                                    xtype: 'button',
                                    id: prototype.idRFNDAssociatedARCR + '-btn-filter',
                                    iconCls: 'prx-icon-filter',
                                    tooltip: 'Hidden/Show filter',
                                    listeners: {
                                        click: 'onFilterClick'
                                    }
                                },
                                {
                                    xtype: 'button',
                                    id: prototype.idRFNDAssociatedARCR + '-btn-excel',
                                    iconCls: 'prx-icon-excel',
                                    tooltip: 'Export to Excel',
                                    listeners: {
                                        click: 'onExcelClick'
                                    }
                                },
                                {
                                    xtype: 'button',
                                    id: prototype.idRFNDAssociatedARCR + '-btn-clear',
                                    iconCls: 'prx-icon-clear',
                                    tooltip: 'Clear Options',
                                    listeners: {
                                        click: 'onClearClick'
                                    }
                                }
                            ]
                        }
                    ]
                },
                {
                    xtype: 'panel',
                    id: prototype.idRFNDAssociatedARCR + '-contenedor-filters',
                    bodyStyle: 'background-color: #E3EAF9;',
                    defaults: {
                        bodyStyle: 'background: transparent',
                        border: false,
                        padding: '5px'
                    },
                    padding: '1px 5px 1px 5px',
                    items: [
                        {
                            xtype: 'form',
                            id: prototype.idRFNDAssociatedARCR + '-contenedor-filters-form',
                            defaults: {
                                padding: '1px',
                                bodyStyle: 'background: transparent'
                            },
                            items: [
                                {
                                    xtype: 'panel',
                                    id: prototype.idRFNDAssociatedARCR + '-box-filter-01',
                                    layout: 'hbox',
                                    border: true,
                                    defaults: {
                                        // style: 'margin-left:1px',
                                        padding: '5px 1px 5px 1px'
                                    },
                                    items: [
                                        {
                                            xtype: 'combo',
                                            fieldLabel: 'Search Type',
                                            id: prototype.idRFNDAssociatedARCR + '-search-type',
                                            labelAlign: 'left',
                                            queryMode: 'local',
                                            valueField: 'code',
                                            displayField: 'name',
                                            emptyText: '[SELECTED]',
                                            labelWidth: 75,
                                            labelClsExtra: 'prx-label-search',
                                            width: 275,
                                            editable: false,
                                            listConfig: {
                                                minWidth: 200
                                            },
                                            listeners: {
                                                afterrender: 'onCmbSearchAfterRender',
                                                change: 'onCmbSearchChange'
                                            }
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtIATA',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            maxLength: 8,
                                            enforceMaxLength: 8,
                                            width: 100,
                                            hideLabel: true,
                                            enableKeyEvents: true,
                                            hidden: true
                                        },
                                        {
                                            xtype: 'datefield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtFilterDateFrom',
                                            fieldLabel: 'From',
                                            format: 'Y/m/d',
                                            maxValue: Ext.Date.format(new Date(), 'Y/m/d'),
                                            value: Ext.Date.format(new Date(), 'Y/m/d'),
                                            labelWidth: 40,
                                            labelAlign: 'right',
                                            width: 135,
                                            hidden: true,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'datefield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtFilterDateTo',
                                            fieldLabel: 'To',
                                            format: 'Y/m/d',
                                            maxValue: Ext.Date.format(new Date(), 'Y/m/d'),
                                            value: Ext.Date.format(new Date(), 'Y/m/d'),
                                            labelWidth: 40,
                                            labelAlign: 'right',
                                            width: 135,
                                            hidden: true,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txthora1',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            fieldLabel: 'Start hour',
                                            labelAlign: 'right',
                                            labelWidth: 60,
                                            width: 100,
                                            hidden: true,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txthora2',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            fieldLabel: 'End hour',
                                            labelAlign: 'right',
                                            labelWidth: 60,
                                            width: 100,
                                            hidden: true,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtCia',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            hideLabel: true,
                                            value: '139',
                                            maxLength: 3,
                                            enforceMaxLength: 3,
                                            width: 35,
                                            hidden: true
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtFrmaSerie',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            hideLabel: true,
                                            width: 80,
                                            hidden: true,
                                            maxLength: 10,
                                            enforceMaxLength: 10,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtSeq',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            hideLabel: true,
                                            width: 30,
                                            maxLength: 2,
                                            enforceMaxLength: 2,
                                            hidden: true
                                        },
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-txtNumber',
                                            maskRe: /^-?[0-9]*(\.[0-9]{1,2})?$/,
                                            hideLabel: true,
                                            width: 80,
                                            hidden: true,
                                            maxLength: 10,
                                            enforceMaxLength: 10,
                                            listeners: {
                                                specialkey: 'onSearchkey'
                                            }
                                        },
                                        {
                                            xtype: 'combo',
                                            id: prototype.idRFNDAssociatedARCR + '-CmbStatus',
                                            fieldLabel: 'Status',
                                            queryMode: 'local',
                                            displayField: 'name',
                                            valueField: 'code',
                                            width: 200,
                                            labelWidth: 50,
                                            labelAlign: 'right',
                                            emptyText: '',
                                            listConfig: {
                                                minWidth: 200
                                            },
                                            hidden: true,
                                            listeners: {
                                                afterrender: 'onCmbStatusAfterRender',
                                                change: 'onCmbStatusChange'
                                            }
                                        }
                                    ]
                                },
                                {
                                    xtype: 'panel',
                                    id: prototype.idRFNDAssociatedARCR + '-box-filter-02',
                                    layout: 'hbox',
                                    style: 'padding: 0px',
                                    border: true,
                                    hidden: true,
                                    defaults: {
                                        // style: 'margin-left:1px'
                                        padding: '5px 1px 5px 1px'
                                    },
                                    items: [
                                        {
                                            xtype: 'textfield',
                                            id: prototype.idRFNDAssociatedARCR + '-cmbCountry',
                                            fieldLabel: 'Country',
                                            maskRe: /[A-Z,a-z,Ñ,ñ]/,
                                            maxLength: 2,
                                            enforceMaxLength: 2,
                                            labelWidth: 50,
                                            width: 110,
                                            listeners: {
                                                specialkey: 'onSearchkey',
                                                change: 'onchange'
                                            }
                                        },
                                        {
                                            xtype: 'combo',
                                            id: prototype.idRFNDAssociatedARCR + '-txtUser',
                                            fieldLabel: 'Auditor',
                                            queryMode: 'local',
                                            displayField: 'A3389REGAS',
                                            valueField: 'A3389REGAS',
                                            width: 200,
                                            labelWidth: 50,
                                            labelAlign: 'right',
                                            emptyText: '',
                                            listConfig: {
                                                minWidth: 200
                                            }
                                        }
                                    ]
                                }
                            ]
                        }
                    ]
                },
                {
                    xtype: 'panel',
                    layout: {
                        type: 'hbox',
                        pack: 'center'
                    },
                    border: true,
                    bodyStyle: 'background-color: #E3EAF9;',
                    defaults: {
                        border: false,
                        padding: '0px 5px 0px 5px'
                    },
                    padding: '1px 5px 1px 5px',
                    items: [
                        {
                            xtype: 'grid',
                            id: prototype.idRFNDAssociatedARCR + '-grid',
                            columnLines: true,
                            autoScroll: true,
                            width: 1360,
                            height: 480,
                            columns: {
                                items: [
                                    {
                                        text: 'Origin',
                                        dataIndex: 'A3647MODO',
                                        width: 80
                                    }, {
                                        text: 'Area',
                                        dataIndex: 'A3647AREA',
                                        width: 100
                                    }, {
                                        text: 'Folio',
                                        dataIndex: 'A3647FOLIO',
                                        width: 80
                                    },
                                    {
                                        text: 'Qty TKT',
                                        dataIndex: 'A3647CANTIDAD',
                                        width: 80
                                    },
                                    {
                                        text: 'Application </br>date',
                                        dataIndex: 'A3647FAPPI',
                                        width: 80
                                    },
                                    {
                                        text: 'Authorise/</br>Reject date',
                                        dataIndex: 'A3647FAUTO',
                                        width: 75
                                    },
                                    {
                                        text: 'Time',
                                        dataIndex: 'A3647HAUTO',
                                        width: 50
                                    },
                                    {
                                        text: 'Country',
                                        dataIndex: 'A3647PAIS',
                                        width: 60
                                    },
                                    {
                                        text: 'Cur.',
                                        dataIndex: 'A3647MDA',
                                        width: 40
                                    },
                                    {
                                        text: 'Net',
                                        dataIndex: 'A3647NETO',
                                        width: 70,
                                        align: 'right'
                                    },
                                    {
                                        text: 'Description',
                                        dataIndex: 'A3647RAAG',
                                        flex: 1,
                                        align: 'left',
                                        renderer: 'onRendererColumnReason'
                                    },
                                    {
                                        text: 'Auditor',
                                        dataIndex: 'A3647REGAS',
                                        width: 90
                                    },
                                    {
                                        text: 'Sales audit',
                                        dataIndex: 'A3647STATO',
                                        flex: 1,
                                        align: 'left'
                                    },
                                    {
                                        text: 'Status',
                                        dataIndex: 'A3647FLAG',
                                        width: 150,
                                        renderer: 'onRendererColumnStatus'
                                    },
                                    {
                                        text: 'Days',
                                        dataIndex: 'A3647DIAS',
                                        width: 45
                                    },
                                    {
                                        text: 'On time',
                                        dataIndex: '',
                                        width: 60,
                                        renderer: 'onRendererColumnOnTime'
                                    },
                                    {
                                        sortable: false,
                                        xtype: 'actioncolumn',
                                        width: 50,
                                        align: 'center',
                                        items: [
                                            {
                                                iconCls: 'prx-icon-detail',
                                                tooltip: 'Detail',
                                                handler: 'onDetailClick'
                                            }
                                        ]
                                    }, {
                                        sortable: false,
                                        xtype: 'actioncolumn',
                                        width: 50,
                                        align: 'center',
                                        items: [
                                            {
                                                iconCls: 'prx-icon-image-process',
                                                tooltip: 'Update',
                                                handler: 'onUpdateClick'
                                            }
                                        ]
                                    }
                                ],
                                defaults: {
                                    sortable: true,
                                    menuDisabled: true,
                                    align: 'center'
                                }
                            },
                            viewConfig: {
                                // trackOver: false,
                                stripeRows: true,
                                enableTextSelection: true
                            }
                        }
                    ]
                },
                {
                    xtype: 'panel',
                    id: prototype.idRFNDAssociatedARCR + '-pagginator-legend',
                    layout: {
                        type: 'hbox',
                        pack: 'center'
                    },
                    border: true,
                    bodyStyle: 'background-color: transparent;',
                    defaults: {
                        border: false,
                        padding: '0px 5px 0px 5px'
                    },
                    padding: '1px 5px 1px 5px',
                    items: [
                        {
                            xtype: 'panel',
                            width: prototype.widthContenedor,
                            height: 25,
                            layout: {
                                type: 'hbox',
                                pack: 'center'
                            },
                            defaults: {
                                xtype: 'label',
                                margin: '3px 0px 0px 5px'
                            },
                            items: [
                                {
                                    text: 'Page',
                                    width: 50
                                },
                                {
                                    id: prototype.idRFNDAssociatedARCR + '-lbl-currentPage',
                                    text: '1',
                                    width: 50
                                },
                                {
                                    text: 'Of',
                                    width: 50
                                },
                                {
                                    id: prototype.idRFNDAssociatedARCR + '-lbl-pageCount',
                                    text: '0',
                                    width: 50
                                },
                                {xtype: 'tbspacer', width: 100},
                                {
                                    text: 'Total found',
                                    width: 80
                                },
                                {
                                    id: prototype.idRFNDAssociatedARCR + '-lbl-total',
                                    text: '0',
                                    width: 50
                                }
                            ]
                        }
                    ]
                }
            ]
        }
    ]
});



