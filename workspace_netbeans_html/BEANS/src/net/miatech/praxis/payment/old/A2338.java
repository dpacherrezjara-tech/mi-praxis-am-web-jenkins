/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package net.miatech.praxis.payment.old;

/**
 *
 * @author ctarazona
 */
public class A2338 {
    public long RN = 0;    
    public String CCUST = "";
    public String TDOC = "";
    public String STVAL = "";
    public String SCOUNTRY = "";
    public String FTE = "";
    public String MERCHN = "";
    public String MERCHNP = "";
    public String STPAGO = "";
    public String MERCHNL = "";
    public String CODEBANK = "";
    public String SDATE = "";
    public String SCARDN = "";
    public String SCARCOD = "";
    public String SAUTHOC = "";
    public String CUOPLAN = "";
    public String SCURRENCY = "";
    public String PLAZO = "";
    public String FPRESENT = "";
    public String CUOTAS = "";
    public String CUOTAV = "";
    public String NUMLIQUI = "";
    public String FVCLEAR = "";
    public int PLAZOPAG = 0;
    public String TPLAZOPA = "";
    public double BRUTO = 0.00;
    public double COMISB = 0.00;
    public double IVA = 0.00;
    public double NETO = 0.00;
    public double TOTDESC = 0.00;
    public double PTASAD = 0.00;
    public double ALICUOTA = 0.00;
    public double IMPORTESD = 0.00;
    public double IMPORTEF = 0.00;
    public double PDESCUEN = 0.00;
    public double ARANCEL = 0.00;
    public double IMPORTCF = 0.00;
    public double IVAF = 0.00;
    public double CTASAD = 0.00;
    public double IVAT = 0.00;
    public double IMPORTOT = 0.00;
    public double IMPORSDE = 0.00;
    public double IMPORFIN = 0.00;
    public double PORDESCU = 0.00;
    public double IMPARANC = 0.00;
    public double IVAARANC = 0.00;
    public double IVACFINA = 0.00;
    public double IMPCTASD = 0.00;
    public double IVACTASD = 0.00;
    //Status de auditoria
    public String STIMPARA = "";
    public String STIVAARA = "";
    public String STIMPOCF = "";
    public String STIVACFI = "";
    public String STFECPAG = "";
    //Campos calculados de auditoria
    public double AIMPARAN = 0.00;     
    public double AIVAARAN = 0.00;     
    public double AIMPORCF = 0.00;     
    public double AIVACFIN = 0.00;     
    public int APLAZOPA = 0;     
    
    public String FECRFILE = "";
    public String FSELEC = "";
    public String FECSELEC = "";
    public String SEQNUM = "";
    public String USCR = "";
    public String FECR = "";
    public String HOCR = "";
    public String USUP = "";
    public String FEUP = "";
    public String HOUP = "";
}
