package net.miatech.praxis.payment.entities;

import java.math.BigDecimal;

/**
 *
 * @author Dvicente
 */
public class A4775 {

    private String CCUST;
    private String PRDA;
    private String COUNTRY;
    private String STVAL;
    private String OPERATNBR;
    private String SUCURNAME;
    private String PNR;
    private String USERCOBRO;
    private String TIPOPAGO;
    private String SCARDN;
    private String SAUTHOC;
    private String NAMECLIEN;
    private String ESTATUS;
    private String TIPOVENTA;
    private String MERCHID;
    private String NAMEMERCH;
    private BigDecimal SVFOP;
    private String SDATE;
    private String STIME;
    private String NAMECARD;
    private String TIPOCARD;
    private String BANCOEMI;
    private String TICKET1;
    private String TICKET2;
    private String TICKET3;
    private String TICKET4;
    private String TICKET5;
    private String TICKET6;
    private String TICKET7;
    private String TICKET8;
    private String TICKET9;
    private String TICKET10;
    private String FSELEC;
    private String FAMEX;
    private String FAMEXCHG;
    private String CHADJNBR;
    private String TRANSID;
    private String AREFNBR;
    private String CERROR;
    private String FREGLA;
    private String STCON;
    private String FCONT;
    private String IDCON;
    private String USCR;
    private String FECR;
    private String HOCR;
    private String PGMCR;
    private String USUP;
    private String FEUP;
    private String HOUP;
    private String PGMUP;

    public String getCCUST() {
        return CCUST;
    }

    public void setCCUST(String CCUST) {
        this.CCUST = CCUST;
    }

    public String getPRDA() {
        return PRDA;
    }

    public void setPRDA(String PRDA) {
        this.PRDA = PRDA;
    }

    public String getCOUNTRY() {
        return COUNTRY;
    }

    public void setCOUNTRY(String COUNTRY) {
        this.COUNTRY = COUNTRY;
    }

    public String getSTVAL() {
        return STVAL;
    }

    public void setSTVAL(String STVAL) {
        this.STVAL = STVAL;
    }

    public String getOPERATNBR() {
        return OPERATNBR;
    }

    public void setOPERATNBR(String OPERATNBR) {
        this.OPERATNBR = OPERATNBR;
    }

    public String getSUCURNAME() {
        return SUCURNAME;
    }

    public void setSUCURNAME(String SUCURNAME) {
        this.SUCURNAME = SUCURNAME;
    }

    public String getPNR() {
        return PNR;
    }

    public void setPNR(String PNR) {
        this.PNR = PNR;
    }

    public String getUSERCOBRO() {
        return USERCOBRO;
    }

    public void setUSERCOBRO(String USERCOBRO) {
        this.USERCOBRO = USERCOBRO;
    }

    public String getTIPOPAGO() {
        return TIPOPAGO;
    }

    public void setTIPOPAGO(String TIPOPAGO) {
        this.TIPOPAGO = TIPOPAGO;
    }

    public String getSCARDN() {
        return SCARDN;
    }

    public void setSCARDN(String SCARDN) {
        this.SCARDN = SCARDN;
    }

    public String getSAUTHOC() {
        return SAUTHOC;
    }

    public void setSAUTHOC(String SAUTHOC) {
        this.SAUTHOC = SAUTHOC;
    }

    public String getNAMECLIEN() {
        return NAMECLIEN;
    }

    public void setNAMECLIEN(String NAMECLIEN) {
        this.NAMECLIEN = NAMECLIEN;
    }

    public String getESTATUS() {
        return ESTATUS;
    }

    public void setESTATUS(String ESTATUS) {
        this.ESTATUS = ESTATUS;
    }

    public String getTIPOVENTA() {
        return TIPOVENTA;
    }

    public void setTIPOVENTA(String TIPOVENTA) {
        this.TIPOVENTA = TIPOVENTA;
    }

    public String getMERCHID() {
        return MERCHID;
    }

    public void setMERCHID(String MERCHID) {
        this.MERCHID = MERCHID;
    }

    public String getNAMEMERCH() {
        return NAMEMERCH;
    }

    public void setNAMEMERCH(String NAMEMERCH) {
        this.NAMEMERCH = NAMEMERCH;
    }

    public BigDecimal getSVFOP() {
        return SVFOP;
    }

    public void setSVFOP(BigDecimal SVFOP) {
        this.SVFOP = SVFOP;
    }

    public String getSDATE() {
        return SDATE;
    }

    public void setSDATE(String SDATE) {
        this.SDATE = SDATE;
    }

    public String getSTIME() {
        return STIME;
    }

    public void setSTIME(String STIME) {
        this.STIME = STIME;
    }

    public String getNAMECARD() {
        return NAMECARD;
    }

    public void setNAMECARD(String NAMECARD) {
        this.NAMECARD = NAMECARD;
    }

    public String getTIPOCARD() {
        return TIPOCARD;
    }

    public void setTIPOCARD(String TIPOCARD) {
        this.TIPOCARD = TIPOCARD;
    }

    public String getBANCOEMI() {
        return BANCOEMI;
    }

    public void setBANCOEMI(String BANCOEMI) {
        this.BANCOEMI = BANCOEMI;
    }

    public String getFSELEC() {
        return FSELEC;
    }

    public void setFSELEC(String FSELEC) {
        this.FSELEC = FSELEC;
    }

    public String getFAMEX() {
        return FAMEX;
    }

    public void setFAMEX(String FAMEX) {
        this.FAMEX = FAMEX;
    }

    public String getFAMEXCHG() {
        return FAMEXCHG;
    }

    public void setFAMEXCHG(String FAMEXCHG) {
        this.FAMEXCHG = FAMEXCHG;
    }

    public String getCHADJNBR() {
        return CHADJNBR;
    }

    public void setCHADJNBR(String CHADJNBR) {
        this.CHADJNBR = CHADJNBR;
    }

    public String getTRANSID() {
        return TRANSID;
    }

    public void setTRANSID(String TRANSID) {
        this.TRANSID = TRANSID;
    }

    public String getAREFNBR() {
        return AREFNBR;
    }

    public void setAREFNBR(String AREFNBR) {
        this.AREFNBR = AREFNBR;
    }

    public String getCERROR() {
        return CERROR;
    }

    public void setCERROR(String CERROR) {
        this.CERROR = CERROR;
    }

    public String getFREGLA() {
        return FREGLA;
    }

    public void setFREGLA(String FREGLA) {
        this.FREGLA = FREGLA;
    }

    public String getSTCON() {
        return STCON;
    }

    public void setSTCON(String STCON) {
        this.STCON = STCON;
    }

    public String getFCONT() {
        return FCONT;
    }

    public void setFCONT(String FCONT) {
        this.FCONT = FCONT;
    }

    public String getIDCON() {
        return IDCON;
    }

    public void setIDCON(String IDCON) {
        this.IDCON = IDCON;
    }

    public String getUSCR() {
        return USCR;
    }

    public void setUSCR(String USCR) {
        this.USCR = USCR;
    }

    public String getFECR() {
        return FECR;
    }

    public void setFECR(String FECR) {
        this.FECR = FECR;
    }

    public String getHOCR() {
        return HOCR;
    }

    public void setHOCR(String HOCR) {
        this.HOCR = HOCR;
    }

    public String getPGMCR() {
        return PGMCR;
    }

    public void setPGMCR(String PGMCR) {
        this.PGMCR = PGMCR;
    }

    public String getUSUP() {
        return USUP;
    }

    public void setUSUP(String USUP) {
        this.USUP = USUP;
    }

    public String getFEUP() {
        return FEUP;
    }

    public void setFEUP(String FEUP) {
        this.FEUP = FEUP;
    }

    public String getHOUP() {
        return HOUP;
    }

    public void setHOUP(String HOUP) {
        this.HOUP = HOUP;
    }

    public String getPGMUP() {
        return PGMUP;
    }

    public void setPGMUP(String PGMUP) {
        this.PGMUP = PGMUP;
    }

    public String getTICKET1() {
        return TICKET1;
    }

    public void setTICKET1(String TICKET1) {
        this.TICKET1 = TICKET1;
    }

    public String getTICKET2() {
        return TICKET2;
    }

    public void setTICKET2(String TICKET2) {
        this.TICKET2 = TICKET2;
    }

    public String getTICKET3() {
        return TICKET3;
    }

    public void setTICKET3(String TICKET3) {
        this.TICKET3 = TICKET3;
    }

    public String getTICKET4() {
        return TICKET4;
    }

    public void setTICKET4(String TICKET4) {
        this.TICKET4 = TICKET4;
    }

    public String getTICKET5() {
        return TICKET5;
    }

    public void setTICKET5(String TICKET5) {
        this.TICKET5 = TICKET5;
    }

    public String getTICKET6() {
        return TICKET6;
    }

    public void setTICKET6(String TICKET6) {
        this.TICKET6 = TICKET6;
    }

    public String getTICKET7() {
        return TICKET7;
    }

    public void setTICKET7(String TICKET7) {
        this.TICKET7 = TICKET7;
    }

    public String getTICKET8() {
        return TICKET8;
    }

    public void setTICKET8(String TICKET8) {
        this.TICKET8 = TICKET8;
    }

    public String getTICKET9() {
        return TICKET9;
    }

    public void setTICKET9(String TICKET9) {
        this.TICKET9 = TICKET9;
    }

    public String getTICKET10() {
        return TICKET10;
    }

    public void setTICKET10(String TICKET10) {
        this.TICKET10 = TICKET10;
    }
    
}
