/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package net.miatech.praxis.flown;

/**
 *
 * @author claudia
 */
public class A1691 {

    public String CCUST = "";
    public String STVAL = "";
    public String CARRI = "";
    public String FFLOW = "";
    public String TOPER = "";
    public String FSENDSS = "";
    public String CDEPART = "";
    public String CARRIVA = "";
    public String ZONE = "";
    public String MINICONEC = "";
    public String LEGSEQ = "";
    public String NFLIGHT = "";
    public String DFLIGHT = "";
    public String NPLANE = "";
    public String FSTASS = "";
    public String LOCDEP = "";
    public String LOCARR = "";
    public String UTCDEP = "";
    public String UTCARR = "";
    public String FCONSIC = "";
    public String FOPERZUL = "";
    public String FMULTI = ""; 
    public String FSENDOD = "";
    public long QCPNOD = 0;
    public long QCPNTOT = 0;
    public String FSTAOD = "";
    public String FSENDVC = "";
    public String FSTAVC = "";
    public long QCPNVC = 0;
    public long QCPNLEG = 0;
    public long QCPNMA = 0;
    public long QCPNOCR = 0;
    public long QCPNON = 0;
    public long QCPNOAL = 0;
    public long QCPHARB = 0;
    public long QCPCFRE = 0;
    public long QCPCABY = 0;
    public long QCPCABF = 0;
    public long QCPAD = 0;
    public long QCPCHD = 0;
    public long QCPINF = 0;
    public long QCPTRA = 0;
    public String FCLOSE = "";
    public long QCPNVAL = 0;
    public long QCPCON = 0;
    public long QCPNCON = 0;
    public double VCPNUSD = 0;
    public double VCPNLOC = 0;
    public String FSTAPO = "";
    public String FCLOFO = "";
    public long QFFLOW = 0;
    public double VCPN = 0; 
    //INFORMATION FLIGHT FISICO   
    public String FSENDFI = "";
    public int QCPNFI = 0;
    public int QCPNFRE = 0;
    public String FSTAFI = "";
    
    //tot
    public long totQCPNLEG=0;
    public double totVCPN = 0; 
    public String RFIC = ""; 
    //Auditoria
    public String USCR = "";
    public String FECR = "";
    public String HOCR = "";
    public String USUP = "";
    public String FEUP = "";
    public String HOUP = "";
}
